    ...
    <section id="work" className="py-20 bg-gray-100 dark:bg-gray-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-24">
          {projects.map((project, index) => (
            <div key={index} className={`grid grid-cols-1 lg:grid-cols-2 gap-8 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
              ...
              <div className={`space-y-4 ${index % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''}`}>
                <div className="text-sm font-['Poppins']" style={{ color: '#0072b1' }}>
                  Featured Project
                </div>
                ...
                {isEditing ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      ...
                      className="w-full p-2 rounded-md bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700"
                    />
                    <button onClick={() => onRemoveProject(index)} className="self-start text-red-600 hover:underline font-['Poppins'] font-bold">Remove</button>
                  </div>
                ) : (
                  <div className="flex space-x-4">
                    {project.github && (
                      <a
                        ...
                        style={{ '--hover-color': '#0072b1' } as React.CSSProperties}
          ...
          {isEditing && (
            <div>
              <button onClick={onAddProject} className="px-4 py-2 rounded-md text-white font-['Poppins'] font-bold" style={{ backgroundColor: '#0072b1' }}>Add Project</button>
            </div>
          )}
        </div>
      </div>
